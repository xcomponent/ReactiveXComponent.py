...
from reactivexcomponent.configuration.serializer import convert_command_data_to_websocket_input_format
...
class Subscriber:
	...
	def get_data_to_send(self,topic,kind):
	...
	def add_subscribe_state_machine(self, component_name, state_machine_name):
		self.subscribed_state_machines[component_name] = (self.subscribed_state_machines).get(component_name, [])
	...
	def send_subscribe_request_to_topic(self, topic, kind):
		data = self.get_data_to_send(topic, kind)
		command_data = {"Command": Command.subscribe, "Data": data}
		input_data = convert_command_data_to_websocket_input_format(command_data)
